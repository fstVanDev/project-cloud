...
const SButton = styled.button<IButtonStyleProps>`
  ...
  background-color: ${({ outline, color }) => (outline ? "transparent" : `colors.blue)`)};
  border: ${({ outline, color }) => (outline ? `1px solid colors.blue)` : "none")};
  color: ${({ outline, color }) => (outline ? `colors.blue)` : `rgb(${colors.white})`)};
  ...
  &:active {
    ...
    box-shadow: ${({ outline }) => (outline ? "none" : `${shadows.soft}`)};
    color: ${({ outline, color }) =>
      outline ? `colors.blue)` : `rgba(${colors.white}, 0.24)`};
  ...
  & ${SIcon} {
    ...
    background-color: ${({ outline, color }) =>
      outline ? `colors.blue)` : `rgb(${colors.white})`};
